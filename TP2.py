...
nu = 2 # tiré de l'équation d'avection
tmax = 2 # temps maximum de calcul à 2 secondes

# Création des formes matricielles
def mat(n,d,vh,vb):
    M = d*np.eye(n) # éléments sur la diagonale
    for i in range(n-1):
        M[i,i+1] = vh # éléments sur la diagonale supérieure
        M[i+1,i] = vb # éléments sur la diagonale inférieure
    M[0,n-1] = vb
    M[n-1,0] = vh
    return(M)
#
...
def solEC(h,tau):
    ...
    for i in range(1,nmax):
        U[i,:] = np.linalg.solve(invM,U[i-1,:]) # trouver un en fonction de un-1
    return(U,x)
#
...
def solIC(h,tau):
    ...
    M = mat(N+1,1,c/2,-c/2)
    for i in range(1,nmax):
        U[i,:] = np.linalg.solve(M,U[i-1,:]) # trouver un en fonction de un-1
    return(U,x)
#
